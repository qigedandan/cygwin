...
module HplCC2420InterruptsP
{
  ...
  uses interface PlatformInterruptRF<TPlatformIntUser> as IntRF;
}
implementation {
...
#define MAKE_NEW_RF_CAPTURE(name, bit_name)	   \
  async command error_t name.captureRisingEdge() { \
    atomic {                                       \
     ...
     RFIM |=  _BV(CC2430_RFIM_##bit_name);	   \
     IEN2 |=  _BV(CC2430_IEN2_RFIE);               \
     ADBG(1, "captureRisingEdge RFIF=0x%x, RFIM=0x%x, IEN0=0x%x, IEN2=0x%x\r\n", (int)RFIF, (int)RFIM, (int)IEN0, (int)IEN2);\
     return SUCCESS;                               \
    }                                              \
  };                                               \
 ...
 */
  ...
  event void IntRF.firedRF()
  {
    ...
    // see datasheet section 14.4
     ...
     ADBG(5, "SIG_RF RFIF=0x%x, RFIM=0x%x\r\n", (int)RFIF, (int)RFIM);
