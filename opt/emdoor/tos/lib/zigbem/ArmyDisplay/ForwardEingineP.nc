...
generic module ForwardEngineP(uint8_t SIZE)
{
	provides {
		interface Send;
		interface Receive;
	...
	uses {
		...
		interface Recieve as SubReceive;
...
implementation
{
	...
	command error_t Send.send(message_t* pMsg, uint8_t len)
	{
		...
		len += sizeof(static_multihop_route_header_t);
	...
	event message_t* SubReceive.receive(message_t* msg, void* payload, uint8_t len) {
		...
		payload += sizeof(static_multihop_route_header_t);
		len -= sizeof(static_multihop_route_header_t);
 ...
 */
	...
	{
		return (call SubPacket.payloadLength(msg) -sizeof(static_multihop_route_header_t));
	}

	command void Packet.setPayloadLength(message_t* msg, uint8_t len)
	{
		call SubPacket.setPayloadLength(msg, len+sizeof(static_multihop_route_header_t));
	}

	command uint8_t Packet.maxPayloadLength() 
	{
		return (call SubPacket.maxPayloadLength() - sizeof(static_multihop_route_header_t));
	}
	...
	command void* Packet.getPayload(message_t* msg, uint8_t* len)
	{
		...
		rval += sizeof(static_multihop_route_header_t);
 ...
 */
 	command uint8_t StaticMHopRouteInfo.getType(message_t* msg)
 	{
 		static_multihop_route_header_t* header = (static_multihop_route_header_t*)call SubPacket.getPayload(msg, NULL);

 		return header->id;
 	}

 	command uint8_t StaticMHopRouteInfo.getSensorType(message_t* msg)
 	{
 		static_multihop_route_header_t* header = (static_multihop_route_header_t*)call SubPacket.getPayload(msg, NULL);
	...
	command uint16_t StaticMHopRouteInfo.getOriginaddr(message_t* msg)
	{
		static_multihop_route_header_t* header = (static_multihop_route_header_t*)call SubPacket.getPayload(msg, NULL);
